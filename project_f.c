...
void updatePassword(User *users);
...
void displayMenu(UserRole role) {
    ...
    if (role == ADMIN) {
        printf("13. Add users\n");
        printf("14. View users\n");
        printf("15. Update password\n");
    }
}
...
void addUser(User *users, int *numUsers) {
    ...
    printf("Enter role for the new user (0 for ADMIN, 1 for EMPLOYEE): ");
    int role;
    scanf("%d", &role);
    users[*numUsers].role = (role == 0) ? ADMIN : EMPLOYEE;
...
void viewUsers(User *users, int numUsers) {
    ...
    for (int i = 0; i < numUsers; i++) {
        printf("%s\t\t%s\n", users[i].username, (users[i].role == ADMIN) ? "ADMIN" : "EMPLOYEE");
    }
}
...
void updatePassword(User *user) {
    ...
    if (strcmp(user->password, currentPassword) == 0) {
        ...
        printf("Incorrect password. Password update failed.\n");
    }
}
